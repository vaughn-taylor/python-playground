...
// 👇 only init mistral chat on the mistral chat page
if (document.body.dataset.page === "mistral-chat") {
    import('./js/mistral_chat.js').then((mod) => {
        mod.initMistralChat();
    });
}

// 👇 only init RAG chat on the RAG chat page
import { initRagChat } from './js/rag_chat.js';

document.addEventListener("DOMContentLoaded", () => {
    if (document.body.dataset.page === "rag-chat") {
        initRagChat();
    }
});
